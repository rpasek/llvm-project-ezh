...
/// EZHConstantIslands - Due to limited PC-relative displacements, EZH
...
class EZHConstantIslands : public MachineFunctionPass {
  ...
  using BBInfoVector = SmallVectorImpl<EZHBasicBlockInfo>;
  using BasicBlockInfo = EZHBasicBlockInfo;
...
public:
  ...
  StringRef getPassName() const override { return EZH_CP_ISLANDS_OPT_NAME; }
...
char EZHConstantIslands::ID = 0;

/// verify - check BBOffsets, BBSizes, alignment of islands
void EZHConstantIslands::verify() {
#ifndef NDEBUG
...
LLVM_DUMP_METHOD void EZHConstantIslands::dumpBBs() {
  LLVM_DEBUG({
    BBInfoVector &BBInfo = BBUtils->getBBInfo();
...
bool EZHConstantIslands::runOnMachineFunction(MachineFunction &mf) {
  ...
  LLVM_DEBUG(dbgs() << "***** EZHConstantIslands: "
  ...
  // EZH expands PseudoBR_JT instructions here. PseudoBR_JT represents a jump
  // table dispatch. We expand it into:
  // 1. LSL_ADD RA, TableReg, IndexReg, 2
  //    (Compute target pointer address: TableReg + IndexReg * 4)
  // 2. LDR PC, RA, 0
...
void EZHConstantIslands::doInitialConstPlacement(
    std::vector<MachineInstr *> &CPEMIs) {
  ...
  for (unsigned i = 0, e = CPs.size(); i != e; ++i) {
    ...
    MachineInstr *CPEMI =
        BuildMI(*BB, InsAt, DebugLoc(), TII->get(EZH::CONSTPOOL_ENTRY))
            .addSym(LitSym)
...
/// Do initial placement of the jump tables. Unlike ARM/Thumb where jump tables
/// are placed inline near the branch instruction to accommodate short-range
/// TBB/TBH instructions, EZH jump tables are placed at the very end of the
/// function in a dedicated basic block, similar to constant pools, because
/// EZH uses standard 32-bit absolute/relative addresses.
void EZHConstantIslands::doInitialJumpTablePlacement(
  ...
  if (BB->empty() || BB->back().getOpcode() != EZH::CONSTPOOL_ENTRY) {
    BB = MF->CreateMachineBasicBlock();
    MF->push_back(BB);
    BB->setMachineBlockAddressTaken();
    BB->setLabelMustBeEmitted();
  }
  ...
  for (auto [i, JumpTableEntry] : llvm::enumerate(JT)) {
    ...
    MachineInstr *CPEMI = BuildMI(*BB, BB->end(), DebugLoc(),
                                  TII->get(EZH::CONSTPOOL_ENTRY))
                              .addSym(LitSym)
                              .addJumpTableIndex(i)
                              .addImm(JTSize);
    CPEMIs.push_back(CPEMI);
    CPEntries[Offset + i].push_back(CPEntry(CPEMI, i, 0));
    JumpTableEntryIndices[i] = Offset + i;
    for (MachineBasicBlock *MBB : JumpTableEntry.MBBs)
...
bool EZHConstantIslands::BBHasFallthrough(MachineBasicBlock *MBB) {
...
EZHConstantIslands::CPEntry *
EZHConstantIslands::findConstPoolEntry(unsigned CPI,
...
void EZHConstantIslands::initializeFunctionInfo(
  ...
  for (MachineBasicBlock &MBB : *MF) {
    ...
    for (MachineInstr &I : MBB) {
      ...
      if (I.getOpcode() == EZH::CONSTPOOL_ENTRY)
        continue;
...
void EZHConstantIslands::updateForInsertedWaterBlock(MachineBasicBlock *NewBB) {
...
MachineBasicBlock *EZHConstantIslands::splitBlockBeforeInstr(MachineInstr *MI) {
...
bool EZHConstantIslands::isWaterInRange(unsigned UserOffset,
...
bool EZHConstantIslands::isCPEntryInRange(MachineInstr *MI, unsigned UserOffset,
...
bool EZHConstantIslands::decrementCPEReferenceCount(unsigned CPI,
                                                    MachineInstr *CPEMI) {
...
unsigned EZHConstantIslands::getCombinedIndex(const MachineInstr *CPEMI) {
...
int EZHConstantIslands::findInRangeCPEntry(CPUser &U, unsigned UserOffset) {
  ...
  for (CPEntry &CPE : CPEs) {
    ...
    if (isCPEntryInRange(UserMI, UserOffset, CPE.CPEMI, U.getMaxDisp(),
      ...
      U.CPEMI = CPE.CPEMI;
      MCSymbol *CloneSym = CPE.CPEMI->getOperand(0).getMCSymbol();
      UserMI->getOperand(1).ChangeToMCSymbol(CloneSym);
...
/// is not adjusted).
void EZHConstantIslands::createNewWater(unsigned CPUserIndex,
                                        unsigned UserOffset,
                                        MachineBasicBlock *&NewMBB) {
...
bool EZHConstantIslands::handleConstantPoolUser(unsigned CPUserIndex,
                                                bool CloserWater) {
  CPUser &U = CPUsers[CPUserIndex];
  MachineInstr *CPEMI = U.CPEMI;
  unsigned CPI = getCombinedIndex(CPEMI);
  ...
  SmallString<32> SymName;
  unsigned OrigCPI = CPEMI->getOperand(1).getIndex();
  if (CPEMI->getOperand(1).isJTI()) {
    OrigCPI = JumpTableEntryIndices[OrigCPI];
  }
  unsigned CloneIdx = CPEntries[CPI].size();
  ...
  U.CPEMI = BuildMI(NewIsland, DebugLoc(), CPEMI->getDesc())
                ...
                .addImm(Size);
  U.MI->getOperand(1).ChangeToMCSymbol(NewLitSym);
...
void EZHConstantIslands::removeDeadCPEMI(MachineInstr *CPEMI) {
...
/// are zero.
bool EZHConstantIslands::removeUnusedCPEntries() {
  unsigned MadeChange = false;
  unsigned NumEntries = CPEntries.size();
  std::vector<bool> IsLive(NumEntries, false);
  std::vector<unsigned> Worklist;

  // Mark all entries directly referenced by instructions as live.
  for (unsigned i = 0; i != NumEntries; ++i) {
    if (!CPEntries[i].empty() && CPEntries[i][0].CPEMI &&
        CPEntries[i][0].RefCount > 0) {
      IsLive[i] = true;
      Worklist.push_back(i);
    }
  }

  // Transitive closure: propagate liveness through EZHConstantPoolValue
  // dependencies.
  ...
  while (!Worklist.empty()) {
    ...
    if (Curr < Constants.size() && Constants[Curr].isMachineConstantPoolEntry()) {
      if (auto *CPV =
              dyn_cast<EZHConstantPoolValue>(Constants[Curr].Val.MachineCPVal)) {
        if (CPV->isConstantPoolIndex()) {
          unsigned DepCPI = CPV->getConstantPoolIndex();
          if (DepCPI < NumEntries && !IsLive[DepCPI]) {
            IsLive[DepCPI] = true;
            Worklist.push_back(DepCPI);
          }
        } else if (CPV->isJumpTable()) {
          unsigned DepJTI = CPV->getJumpTableIndex();
          if (JumpTableEntryIndices.count(DepJTI)) {
            unsigned DepCPI = JumpTableEntryIndices[DepJTI];
            if (DepCPI < NumEntries && !IsLive[DepCPI]) {
              IsLive[DepCPI] = true;
              Worklist.push_back(DepCPI);
            }
  ...
  // Erase any entry that is not marked live.
  for (unsigned i = 0; i != NumEntries; ++i) {
    for (CPEntry &CPE : CPEntries[i]) {
      if (!IsLive[i] && CPE.CPEMI) {
...
bool EZHConstantIslands::fixupUnconditionalBr(ImmBranch &Br) {
  ...
  BuildMI(*MBB, MI, MI->getDebugLoc(), TII->get(EZH::LOAD_CONSTANT),
          EZH::PC)
      .addImm(CPI);
  ...
  // We inserted: 1 (LDR PC) instruction (Size = 4).
  // We removed: 1 instruction (E_GOTO, Size = 4).
  // Net Growth: 0 bytes.
  unsigned Growth = 0;
  BBInfo[MBB->getNumber()].Size += Growth;
