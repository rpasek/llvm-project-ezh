//===-- EZHDelaySlotFiller.cpp - EZH delay slot filler ----------------===//
//
...
struct Filler : public MachineFunctionPass {
  ...
  StringRef getPassName() const override { return "EZH Delay Slot Filler"; }

  bool runOnMachineBasicBlock(MachineBasicBlock &MBB);

  bool runOnMachineFunction(MachineFunction &MF) override {
    const EZHSubtarget &Subtarget = MF.getSubtarget<EZHSubtarget>();
...
// createEZHDelaySlotFillerPass - Returns a pass that fills in delay
// slots in EZH MachineFunctions
FunctionPass *
llvm::createEZHDelaySlotFillerPass(const EZHTargetMachine & /*tm*/) {
  return new Filler();
}
  ...
  for (MachineBasicBlock::instr_iterator I = MBB.instr_begin();
    ...
    if (I->getDesc().hasDelaySlot()) {
      ...
      if (I->getOpcode() == EZH::RET) {
        ...
        assert(RI->getOpcode() == EZH::LDW_RI && RI->getOperand(0).isReg() &&
               RI->getOperand(0).getReg() == EZH::FP &&
        ...
        if (!NopDelaySlotFiller && findDelayInstr(MBB, I, J)) {
          ...
          BuildMI(MBB, std::next(I), DebugLoc(), TII->get(EZH::NOP));
        }
        ++FilledSlots;
      }
