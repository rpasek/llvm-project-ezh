...
void EZHFrameLowering::determineFrameLayout(MachineFunction &MF) const {
  MachineFrameInfo &MFI = MF.getFrameInfo();
  const EZHRegisterInfo *LRI = STI.getRegisterInfo();
...
// ADJDYNALLOC pseudo instructions with a EZH:ADDI with the
// maximum call frame size as the immediate.
void EZHFrameLowering::replaceAdjDynAllocPseudo(MachineFunction &MF) const {
  const EZHInstrInfo &LII = *STI.getInstrInfo();
  unsigned MaxCallFrameSize = MF.getFrameInfo().getMaxCallFrameSize();

  for (MachineBasicBlock &MBB : MF) {
    for (MachineInstr &MI : llvm::make_early_inc_range(MBB)) {
      if (MI.getOpcode() == EZH::ADJDYNALLOC) {
        DebugLoc DL = MI.getDebugLoc();
        Register Dst = MI.getOperand(0).getReg();
        Register Src = MI.getOperand(1).getReg();

        BuildMI(MBB, MI, DL, LII.get(EZH::ADD_I_LO), Dst)
            .addReg(Src)
...
void EZHFrameLowering::emitPrologue(MachineFunction &MF,
  ...
  // Push old FP
  // st %fp,-4[*%sp]
  BuildMI(MBB, MBBI, DL, LII.get(EZH::SW_RI))
      .addReg(EZH::FP)
      .addReg(EZH::SP)
      .addImm(-4)
  ...
  // Generate new FP
  // add %sp,8,%fp
  BuildMI(MBB, MBBI, DL, LII.get(EZH::ADD_I_LO), EZH::FP)
      .addReg(EZH::SP)
      .addImm(8)
      .setMIFlag(MachineInstr::FrameSetup);
  ...
  // sub %sp,StackSize,%sp
  if (StackSize != 0) {
    BuildMI(MBB, MBBI, DL, LII.get(EZH::SUB_I_LO), EZH::SP)
        .addReg(EZH::SP)
        .addImm(StackSize)
        .setMIFlag(MachineInstr::FrameSetup);
  }
...
void EZHFrameLowering::emitEpilogue(MachineFunction & /*MF*/,
  ...
  BuildMI(MBB, MBBI, DL, LII.get(EZH::ADD_I_LO), EZH::SP)
      .addReg(EZH::FP)
      .addImm(0);

  // Restore the frame pointer from the stack.
  BuildMI(MBB, MBBI, DL, LII.get(EZH::LDW_RI), EZH::FP)
      .addReg(EZH::FP)
      .addImm(-8)
      .addImm(LPAC::ADD);
}
