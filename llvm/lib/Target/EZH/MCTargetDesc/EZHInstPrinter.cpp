...
//
// Description:
//   Implements textual assembly printing for MCInst objects, handling
//   preceding condition codes and custom operand formats.
//
// Copied From:
...
//
// Changes:
//   Implemented operand printing matching fsl_smartdma_prv.h syntax; printed
//   condition codes preceding instruction mnemonics (e.g., e_goto_ca).
//
//===----------------------------------------------------------------------===//
...
void EZHInstPrinter::printPerAddrOperand(const MCInst *MI, unsigned OpNo,
                                         raw_ostream &O) {
  const MCOperand &Op = MI->getOperand(OpNo);
  if (Op.isImm()) {
...
void EZHInstPrinter::printPredicateOperand(const MCInst *MI, unsigned OpNum,
                                           raw_ostream &O) {
  unsigned CC = MI->getOperand(OpNum).getImm();
  O << EZHCC::ezhCondCodeToString(static_cast<EZHCC::CondCode>(CC));
}
