...
class EZHAsmParser : public MCTargetAsmParser {
  // Parse operands
  std::unique_ptr<EZHOperand> parseRegister(bool RestoreOnFailure = false);

  std::unique_ptr<EZHOperand> parseImmediate();

  std::unique_ptr<EZHOperand> parseIdentifier();
...
// EZHOperand - Instances of this class represented a parsed machine
// instruction
...
public:
  ...
  bool isHiImm16() {
    ...
    if (const auto *SymbolRefExpr = dyn_cast<MCSpecifierExpr>(Imm.Value))
      return SymbolRefExpr->getSpecifier() == EZH::S_ABS_HI;
  ...
  bool isLoImm16() {
    ...
    // Symbolic reference expression
    if (const auto *SymbolRefExpr = dyn_cast<MCSpecifierExpr>(Imm.Value))
      return SymbolRefExpr->getSpecifier() == EZH::S_ABS_LO;
  ...
  bool isLoImm21() {
    ...
    // Symbolic reference expression
    if (const auto *SymbolRefExpr = dyn_cast<MCSpecifierExpr>(Imm.Value))
      return SymbolRefExpr->getSpecifier() == EZH::S_None;
  ...
  void addLoImm16Operands(MCInst &Inst, unsigned N) const {
    ...
    if (const MCConstantExpr *ConstExpr = dyn_cast<MCConstantExpr>(getImm()))
      ...
      assert(SymbolRefExpr && SymbolRefExpr->getSpecifier() == EZH::S_ABS_LO);
#endif
...
#ifndef NDEBUG
      const auto *SymbolRefExpr = dyn_cast<MCSpecifierExpr>(getImm());
      assert(SymbolRefExpr && SymbolRefExpr->getSpecifier() == EZH::S_ABS_HI);
#endif
      Inst.addOperand(MCOperand::createExpr(getImm()));
    } else if (isa<MCBinaryExpr>(getImm())) {
#ifndef NDEBUG
...
#ifndef NDEBUG
      const auto *SymbolRefExpr = dyn_cast<MCSpecifierExpr>(getImm());
      assert(SymbolRefExpr && SymbolRefExpr->getSpecifier() == EZH::S_None);
#endif
      Inst.addOperand(MCOperand::createExpr(getImm()));
  ...
  static std::unique_ptr<EZHOperand>
  MorphToMemImm(std::unique_ptr<EZHOperand> Op) {
  ...
  static std::unique_ptr<EZHOperand>
  MorphToMemRegReg(MCRegister BaseReg, std::unique_ptr<EZHOperand> Op,
                   unsigned AluOp) {
  ...
  static std::unique_ptr<EZHOperand>
  MorphToMemRegImm(MCRegister BaseReg, std::unique_ptr<EZHOperand> Op,
                   unsigned AluOp) {
    const MCExpr *Imm = Op->getImm();
...
EZHAsmParser::parseRegister(bool RestoreOnFailure) {
  ...
  if (Lexer.getKind() == AsmToken::Identifier) {
    ...
    return EZHOperand::createReg(Reg, Start, End);
  }
  if (PercentTok && RestoreOnFailure)
    Lexer.UnLex(*PercentTok);
  return nullptr;
...
bool EZHAsmParser::parseRegister(MCRegister &RegNum, SMLoc &StartLoc,
  ...
  std::unique_ptr<EZHOperand> Op = parseRegister(/*RestoreOnFailure=*/false);
  if (Op != nullptr)
    RegNum = Op->getReg();
  return (Op == nullptr);
}
...
ParseStatus EZHAsmParser::tryParseRegister(MCRegister &Reg, SMLoc &StartLoc,
  ...
  StartLoc = Tok.getLoc();
  EndLoc = Tok.getEndLoc();
  std::unique_ptr<EZHOperand> Op = parseRegister(/*RestoreOnFailure=*/true);
  if (Op == nullptr)
...
std::unique_ptr<EZHOperand> EZHAsmParser::parseIdentifier() {
  SMLoc Start = Parser.getTok().getLoc();
  SMLoc End = SMLoc::getFromPointer(Parser.getTok().getLoc().getPointer() - 1);
  const MCExpr *Res, *RHS = nullptr;
  auto Kind = EZH::S_None;
  ...
  return EZHOperand::createImm(Res, Start, End);
}

std::unique_ptr<EZHOperand> EZHAsmParser::parseImmediate() {
  SMLoc Start = Parser.getTok().getLoc();
  SMLoc End = SMLoc::getFromPointer(Parser.getTok().getLoc().getPointer() - 1);
  ...
  case AsmToken::Dot:
    if (!Parser.parseExpression(ExprVal))
      return EZHOperand::createImm(ExprVal, Start, End);
    [[fallthrough]];
  default:
    return nullptr;
  }
...
unsigned EZHAsmParser::parseAluOperator(bool PreOp, bool PostOp) {
  StringRef IdString;
  Parser.parseIdentifier(IdString);
  unsigned AluCode = LPAC::stringToEZHAluCode(IdString);
...
bool EZHAsmParser::parsePrePost(StringRef Type, int *OffsetValue) {
...
bool shouldBeSls(const EZHOperand &Op) {
  ...
  if (const MCBinaryExpr *BinaryExpr = dyn_cast<MCBinaryExpr>(Op.getImm())) {
    ...
    return (LHSSymbolRefExpr &&
            LHSSymbolRefExpr->getSpecifier() == EZH::S_None);
  }
  return false;
}

// Matches memory operand. Returns true if error encountered.
ParseStatus EZHAsmParser::parseMemoryOperand(OperandVector &Operands) {
  ...
  if (Operands[0]->isToken())
    Type = static_cast<EZHOperand *>(Operands[0].get())->getToken();
  ...
  std::unique_ptr<EZHOperand> Op = parseRegister();
  if (!Op)
    Op = parseImmediate();
  ...
  Parser.Lex(); // Eat the '['.
  std::unique_ptr<EZHOperand> Offset = nullptr;
  if (Op)
    Offset.swap(Op);
  ...
  if (!Op) {
    if (!Offset) {
      if ((Op = parseImmediate()) && Lexer.is(AsmToken::RBrac)) {
        ...
        // SLS, the rest as RM.
        if (shouldBeSls(*Op)) {
          Operands.push_back(EZHOperand::MorphToMemImm(std::move(Op)));
        } else {
  ...
  if (Lexer.is(AsmToken::RBrac)) {
    ...
    if (!Offset) {
      ...
      Offset = EZHOperand::createImm(OffsetConstExpr, Start, End);
    }
  } else {
  ...
  Operands.push_back(
      Offset->isImm()
          ? EZHOperand::MorphToMemRegImm(BaseReg, std::move(Offset), AluOp)
          : EZHOperand::MorphToMemRegReg(BaseReg, std::move(Offset), AluOp));
...
StringRef EZHAsmParser::splitMnemonic(StringRef Name, SMLoc NameLoc,
  ...
  if (Mnemonic[0] == 'b' ||
    ...
    LPCC::CondCode CondCode =
        LPCC::suffixToEZHCondCode(Mnemonic.substr(1, Next));
    if (CondCode != LPCC::UNKNOWN) {
      Mnemonic = Mnemonic.slice(0, 1);
      Operands->push_back(EZHOperand::CreateToken(Mnemonic, NameLoc));
      Operands->push_back(EZHOperand::createImm(
          MCConstantExpr::create(CondCode, getContext()), NameLoc, NameLoc));
      if (IsBRR) {
        Operands->push_back(EZHOperand::CreateToken(".r", NameLoc));
      }
      return Mnemonic;
    }
  }
  ...
  if (Mnemonic.starts_with("sel") ||
    ...
    LPCC::CondCode CondCode = LPCC::suffixToEZHCondCode(Mnemonic);
    if (CondCode != LPCC::UNKNOWN) {
...
static bool IsMemoryAssignmentError(const OperandVector &Operands) {
  ...
  if (EZHOperand *PossibleAluOp =
          static_cast<EZHOperand *>(Operands[PossibleAluOpIdx].get()))
...
static bool IsRegister(const MCParsedAsmOperand &op) {
  return static_cast<const EZHOperand &>(op).isReg();
}
...
static bool MaybePredicatedInst(const OperandVector &Operands) {
  ...
  return StringSwitch<bool>(
             static_cast<const EZHOperand &>(*Operands[0]).getToken())
...
bool EZHAsmParser::parseInstruction(ParseInstructionInfo & /*Info*/,
  ...
  if (Lexer.is(AsmToken::EndOfStatement) && Name.starts_with("bt") &&
    ...
    Operands.insert(Operands.begin(), EZHOperand::CreateToken("bt", NameLoc));
  }
