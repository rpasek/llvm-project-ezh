...
//===----------------------------------------------------------------------===//
// EZHDAGToDAGISel - EZH specific code to select EZH machine
...
// Helper functions for ComplexPattern used on EZHInstrInfo
// Used on EZH Load/Store instructions.
bool EZHDAGToDAGISel::selectAddrSls(SDValue Addr, SDValue &Offset) {
  ...
  if (Addr.getOpcode() == ISD::OR &&
      Addr.getOperand(1).getOpcode() == EZHISD::SMALL) {
  ...
  if (ConstantSDNode *CN = dyn_cast<ConstantSDNode>(Addr)) {
    if (RiMode) {
      ...
      if (isInt<16>(CN->getSExtValue())) {
        ...
        Offset = CurDAG->getTargetConstant(Imm, DL, CN->getValueType(0));
        Base = CurDAG->getRegister(EZH::R0, CN->getValueType(0));
...
namespace llvm {
namespace LPAC {
static AluCode isdToEZHAluCode(ISD::NodeType Node_type) {
  switch (Node_type) {
  case ISD::ADD:
    return AluCode::ADD;
  case ISD::ADDE:
...
bool EZHDAGToDAGISel::selectAddrRr(SDValue Addr, SDValue &R1, SDValue &R2,
  ...
  ISD::NodeType AluOperator = static_cast<ISD::NodeType>(Addr.getOpcode());
  LPAC::AluCode AluCode = LPAC::isdToEZHAluCode(AluOperator);
  if (AluCode != LPAC::UNKNOWN) {
    ...
    if (Addr.getOperand(0).getOpcode() == EZHISD::HI ||
        Addr.getOperand(0).getOpcode() == EZHISD::LO ||
        Addr.getOperand(0).getOpcode() == EZHISD::SMALL ||
        Addr.getOperand(1).getOpcode() == EZHISD::HI ||
        Addr.getOperand(1).getOpcode() == EZHISD::LO ||
        Addr.getOperand(1).getOpcode() == EZHISD::SMALL)
      return false;
