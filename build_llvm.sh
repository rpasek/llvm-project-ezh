...
COMMON_CMAKE_ARGS=(
    ...
    -DCMAKE_ASM_COMPILER_TARGET=ezh-none-elf
    -DCMAKE_C_FLAGS="${COMMON_C_FLAGS} -nostdlibinc \
        -isystem ${ROOT_DIR}/ezh/libc_stubs \
        -isystem ${ROOT_DIR}/build/libc/libc/include"
    -DCMAKE_CXX_FLAGS="${COMMON_C_FLAGS} -fexceptions -frtti \
        -D_LIBCPP_PROVIDES_DEFAULT_RUNE_TABLE -nostdlibinc \
        -isystem ${ROOT_DIR}/ezh/libc_stubs \
        -isystem ${ROOT_DIR}/build/libc/libc/include \
        -I${ROOT_DIR}/libcxxabi/include"
...
# Rebuild libc_nano inside build/libc_nano (disabling float printf and scanf to
# prevent 20KB+ code space bloat on 32KB targets)
