...
//
// Description:
//   Implements thread register context creation, SIGINT stop reason
//   calculation, and post-stop cache invalidation for UnwindLLDB.
//
//===----------------------------------------------------------------------===//
#include "ThreadEZH.h"
...
bool ThreadEZH::CalculateStopInfo() {
  ...
  // Set EZH's stop reason cleanly to Signal 2 (SIGINT) representing a debugger
  // interrupt halt!
...
void ThreadEZH::RefreshStateAfterStop() {
  // Call base class to invalidate register context
  ThreadGDBRemote::RefreshStateAfterStop();
  if (m_reg_context_sp)
    m_reg_context_sp->InvalidateAllRegisters();
  // Forcefully clear cached stack frames so UnwindLLDB re-fetches them
  // using fresh target memory reads.
  ClearStackFrames();
}
