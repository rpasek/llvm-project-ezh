...
bool RegisterContextEZH::ReadAllRegisterValuesBytes() {
  ...
  bytes_read = ezh_process->DoReadMemoryDirect(debug_frame_addr, &sp_addr, 4,
                                               error);
  if (error.Fail()) {
    char dbg_err[256];
  ...
  ezh_process->InvalidateMemoryCache();
  bytes_read = ezh_process->DoReadMemoryDirect(sp_addr - EZH_FRAME_SIZE,
                                               m_reg_values, EZH_FRAME_SIZE,
                                               error);
  if (error.Fail()) {
    char dbg_err[256];
    snprintf(dbg_err, sizeof(dbg_err),
...
bool RegisterContextEZH::ReadRegister(const RegisterInfo *reg_info,
  ...
  if (reg_idx == EZH_REG_IDX_PC) {
    if ((reg_val & EZH_SW_BP_MAGIC_PC_MASK) == EZH_SW_BP_MAGIC_PC_MASK && ezh_process) {
      uint32_t slot = reg_val & EZH_SW_BP_SLOT_MASK;
      lldb::addr_t sw_bp_addr = ezh_process->GetSnapshotSoftwareBreakpointAddr(slot);
