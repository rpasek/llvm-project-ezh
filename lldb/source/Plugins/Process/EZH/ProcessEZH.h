...
#define EZH_MAX_SW_BREAKPOINTS 16
...
public:
  ...
  lldb::addr_t GetActiveSoftwareBreakpointAddr(uint32_t slot) const {
    return (slot < EZH_MAX_SW_BREAKPOINTS) ? m_active_sw_breakpoints[slot]
                                           : LLDB_INVALID_ADDRESS;
  }
  lldb::addr_t GetSnapshotSoftwareBreakpointAddr(uint32_t slot) const {
    return (slot < EZH_MAX_SW_BREAKPOINTS)
               ? m_halt_snapshot_sw_breakpoints[slot]
               : LLDB_INVALID_ADDRESS;
  }
...
private:
  ...
  lldb::addr_t m_active_sw_breakpoints[EZH_MAX_SW_BREAKPOINTS];
  lldb::addr_t m_halt_snapshot_sw_breakpoints[EZH_MAX_SW_BREAKPOINTS];
