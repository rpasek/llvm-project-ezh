...
// Shared EZH Target & Debugger Interface Symbols:
//
// __ezh_debug_frame:
//   A volatile RAM variable defined in startup code (crt0.c) that holds the
//   target's stack frame pointer when halted. When entering a debug halt, the
//   core writes its current SP (pointing to the 68-byte register dump) to this
//   variable and spins in an idle loop waiting for it to be reset to 0. LLDB
//   looks up this symbol to read and write CPU registers during a breakpoint or
//   interrupt stop.
extern volatile uint32_t __ezh_debug_frame;

// __ezh_debug_common:
//   The common naked assembly handler function defined in crt0.c for bitslice
//   interrupts and software debug traps. It saves all 17 CPU registers onto the
//   stack, stores SP into __ezh_debug_frame, and enters the virtual halt loop.
//   LLDB uses this symbol's address to construct 2-instruction redirect
//   sequences when inserting software breakpoints into instruction memory.
...
// Encodes subroutine call (gosub) instruction
constexpr uint32_t EncodeGosub(uint32_t target_address) {
  return (target_address & EZH_WORD_ALIGN_MASK_30BIT) | EZH_OPC_GOSUB;
}

// Encodes str raddr, rdata, offset_bytes instruction
// (offset in bytes, word-aligned)
constexpr uint32_t EncodeStr(uint32_t raddr, uint32_t rdata,
                             int32_t offset_bytes) {
  int32_t offset_words = offset_bytes / 4;
  uint32_t offset8s = static_cast<uint32_t>(offset_words) & 0xFF;
  return EZH_OPC_STR | (0 << 19) | ((raddr & EZH_REG_MASK_4BIT) << 14) |
         ((rdata & EZH_REG_MASK_4BIT) << 20) | (offset8s << 24) | (0 << 10) |
         (1 << 18) | (0 << 11) | (EZH_CC_EU << 5);
}
...
constexpr EZHGoto DecodeGoto(uint32_t inst_val) {
  ...
  op.imm21 =
      (inst_val >> EZH_INSTR_IMM_ADDR_SHIFT) & EZH_PAGE_OFFSET_MASK_21BIT;
...
constexpr EZHMov DecodeMov(uint32_t inst_val) {
  EZHMov op{};
  op.rs1 = (inst_val >> EZH_INSTR_REG_SRC_SHIFT) & EZH_REG_MASK_4BIT;
  int32_t imm = static_cast<int32_t>((inst_val >> EZH_INSTR_IMM_MATH_SHIFT) &
                                     EZH_IMM11_MASK);
...
constexpr EZHPerWrite DecodePerWrite(uint32_t inst_val) {
  ...
  op.per_addr =
      ((inst_val >> 12) & 0x000FF000U) | ((inst_val >> 8) & 0x00000FFCU);
  return op;
}
...
constexpr EZHModifiers DecodeModifiers(uint32_t inst_val) {
  ...
  op.shift_type = static_cast<EZHShiftType>((inst_val >> EZH_SHIFT_TYPE_SHIFT) &
                                            EZH_SHIFT_TYPE_MASK);
