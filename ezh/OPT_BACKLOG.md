...
STATUS (8d4c5674, verified 2026-07-16): PARTIAL — only SINGLE-REGISTER-result libcalls tail-call today. `mul_i32` → `goto __mulsi3` ✓; but soft-float `add_f32` → `pushd ra / gosub __addsf3 / popd pc` and i64-result `div_i64` → `pushd ra / gosub __divdi3 / popd pc` both still keep the frame. The remaining work is TWO distinct paths, not one "register-pair" case: (1) i64-result libcalls (__divdi3 etc.) return a register PAIR R0:R1, so isUsedByReturnOnly must accept both CopyToReg nodes feeding the return, not just a single R0. (2) Soft-float f32 (__addsf3) already returns in a SINGLE register R0 -- it is NOT a register-pair problem; it fails to tail-call because the f32 result reaches the return through a BITCAST (f32<->i32 in the softened DAG) that isUsedByReturnOnly does not look through. Fixing (2) is a small BITCAST-transparency change independent of (1). Also note: passes 31/31 host tests and codegen is correct, but the win is ZERO `.text` in the DEFAULT (bitslice-interrupts ON) corpus. In bitslice mode the tail call still becomes a `goto` (opt fires), but the injected `gotol_bs bitslice_handler` poll writes RA, so RA must be saved/restored around it — the frame becomes `pushd ra / gotol_bs / popd ra / goto __mulsi3` (4 insns), the same count as the un-optimized `pushd ra / gotol_bs / gosub / popd pc`. The `goto`-for-`gosub` swap nets zero bytes. The 2-insn/8-byte win is realized only under `-mno-ezh-bitslice-interrupts`, where the whole frame collapses to a bare `goto __mulsi3`.
...
OUTCOME: ported (whitelist-closed classifier; dedicated OUTLINE_CALL with Defs=[RA] only; RA-dead-and-spilled gate rejects frameless leaves; runs before addPreEmitPass2). Default-on at -Oz ONLY (hasMinSize); available anywhere via -mllvm -enable-machine-outliner. There is NO RA-spill/RegSave fallback -- EZH has a single link register, so a candidate is simply dropped when RA is not already dead across it. MEASURED BENEFIT IS SMALL: the estimate below was a cross-corpus upper bound the outliner cannot reach (it captures only intra-module duplication); actual saving is ~80 bytes on the self-test and exactly 768 bytes across the C tests (12 C executables changed) when forced on at -Os. The 768-byte figure and the "12 executables changed" are C-only -- but not because C++ was skipped: RERUN 2026-07-16 with the CXX-flags gap fixed (EXTRA_CFLAGS="-mllvm -enable-machine-outliner=always" reaching both C and C++, confirmed in CMakeCache) forced the outliner over the WHOLE corpus, C and C++ alike. Result: 3078/3078 on silicon at -O0+-Os -- forced-mode correctness now proven for C AND C++. The C++ tests produced ZERO outlined functions even forced, because the SingleSource/Regression/C++ corpus has no intra-module instruction duplication, not because the outliner is C-only: it operates on machine instructions and is language-agnostic (a synthetic C++ source with a repeated sequence outlines 18 call sites when forced). So the C++ forced-outlining size contribution is genuinely 0 on this corpus, and 768 bytes stands as the full measured saving. Correctness proven four ways: 30/30 host tests; the on-silicon differential (run_outliner.sh: ON vs OFF vs host golden, 1 outlined helper + 24 call sites, byte-identical); the original C-forced -Os corpus run; and the 2026-07-16 C+C++-forced -Os corpus run (3078/3078). Artifact: ezh/llvm_test/ezh_lit_results_outliner_forced.json (now C+C++-forced). Original estimate below is retained for context only.
