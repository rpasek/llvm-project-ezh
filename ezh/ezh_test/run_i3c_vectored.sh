...
#===----------------------------------------------------------------------===#
# TWO-SOURCE VECTORED DISPATCH ("NVIC emulation") -- one EZH core serves two
# interrupt sources concurrently via acc_vectored_hold:
#
#   board B (slave): i3c_vectored_slave.c -- sleeps in acc_vectored_hold with
#       slice 0 = I3C0 IRQ            -> i3c handler (the M3 SDR slave)
#       slice 1 = software doorbell   -> tile handler (16-byte tile copies,
#                 standing in for a display/tiling trigger)
#     The hardware computes the winning source's vector; the loop dispatches
#     to the matching C handler.
#   board A (master): i3c_sdr_master.c (unchanged M3 master) -- RSTDAA,
#     ENTDAA assigning dynamic address 0x30, 17-byte private SDR write.
#
# The runner fires doorbell #1 before the I3C traffic and #2/#3 after it, so
# the same sleeping core is shown vectoring to both handlers in one run.
# Same J18 wiring as the other demos.
#===----------------------------------------------------------------------===#
set -e
cd "$(dirname "$0")"
WT="$(cd ../.. && pwd)"
...
mkdir -p out_i3c_vec; cd out_i3c_vec
...
for t in i3c_vectored_slave i3c_sdr_master; do
  $B/clang $F -I .. -I ../.. ../$t.c -c -o $t.o
  $B/ld.lld -T ../../smartdma.ld --gc-sections --discard-locals crt0.o $t.o minrt.o -o $t.elf
done
echo "built out_i3c_vec/{i3c_vectored_slave,i3c_sdr_master}.elf"
cd ..
...
def i3c_common_init(o):
    ...
    # time-control (slow) clock: the CCC/DAA engine is dead without it
    ...
    # full-drive pins (weak drive corrupts the CCC/DAA open-drain turnarounds)
    o.cmd("mww 0x40004174 0x000001C1"); o.cmd("mww 0x40004178 0x000001C1")  # PIO2_29/30
    o.cmd("mww 0x4000417C 0x00000181")                             # PIO2_31 = I3C0_PUR
...
def i3c_slave_init(o):
    ...
    o.cmd("mww 0x40027048 0x00000080")                             # PENDTRAP hygiene (as in the dispatch proof)

def doorbell(o,ch):
    # REQ latches: drop it first so the re-fire is a genuine rising edge for
    # the slice (a real peripheral trigger produces fresh edges by itself).
    o.cmd("mww 0x40027048 0x%08x"%((1<<ch)<<16))                   # PENDTRAP EN(ch), REQ low
    o.cmd("mww 0x40027048 0x%08x"%(((1<<ch)<<16)|(1<<ch)))         # PENDTRAP EN(ch)|REQ(ch)

SL=A("out_i3c_vec/i3c_vectored_slave.elf"); MA=A("out_i3c_vec/i3c_sdr_master.elf")
S=syms(SL,{"s_buf","s_vec0","s_vec1","s_spur","s_rx","s_stops","s_dachg","s_dyn","s_tiles","tile_dst","exc_signal"})
M=syms(MA,{"m_stage","m_nacked","m_holds","m_complete","exc_signal"})
EXPECT=[0x00]+[0x10+i for i in range(16)]
TILES=3; TILE=16
TILE_EXPECT=[(0xA0+i)&0xff for i in range(TILES*TILE)]

print("=== TWO-SOURCE VECTORED DISPATCH: I3C (slice 0) + tile doorbell (slice 1) ===")
a=OCD(4444); a.cmd("poll off"); pokes(a); i3c_master_init(a)
b=OCD(4445); b.cmd("poll off"); pokes(b); i3c_slave_init(b)
time.sleep(0.3)
start(b,SL); time.sleep(1.0)          # slave parked in acc_vectored_hold
doorbell(b,1); time.sleep(0.4)        # tile #1 while the bus is idle
t1=rd(b,S["s_tiles"])
start(a,MA); time.sleep(3.0)          # the whole I3C ceremony (slice 0 wakes)
doorbell(b,1); time.sleep(0.3)        # tiles #2 and #3 after the transfer
doorbell(b,1); time.sleep(0.4)
m={k:rd(a,M[k]) for k in M}
del a; time.sleep(0.3)
exc=rd(b,S["exc_signal"])
vals={k:rd(b,S[k]) for k in ("s_vec0","s_vec1","s_spur","s_rx","s_stops","s_dachg","s_dyn","s_tiles")}
got=[rd(b,S["s_buf"]+i)&0xff for i in range(len(EXPECT))]
tiles=[rd(b,S["tile_dst"]+i)&0xff for i in range(TILES*TILE)]

print("  MASTER exc=0x%08x stage=%d nacked=0x%x complete=%d"%(m["exc_signal"],m["m_stage"],m["m_nacked"],m["m_complete"]))
print("  SLAVE  exc=0x%08x | vec0(i3c)=%d vec1(tile)=%d spur=%d"%(exc,vals["s_vec0"],vals["s_vec1"],vals["s_spur"]))
print("         i3c: rx=%d stops=%d dachg=%d dynaddr=0x%02x"%(vals["s_rx"],vals["s_stops"],vals["s_dachg"],vals["s_dyn"]))
print("         tiles: %d done (first fired pre-I3C: %d) dst=%s..."%(vals["s_tiles"],t1," ".join("%02x"%x for x in tiles[:8])))
print("         buf=%s"%" ".join("%02x"%x for x in got))
ok = (exc==0xCAFEBABE and m["exc_signal"]==0xCAFEBABE and m["m_nacked"]==0 and m["m_complete"]==1
      and got==EXPECT and vals["s_rx"]==len(EXPECT) and vals["s_stops"]>=1
      and vals["s_dyn"]==0x61 and vals["s_tiles"]==TILES and tiles==TILE_EXPECT
      and vals["s_vec1"]==TILES and vals["s_vec0"]>=2 and t1>=1)
if got!=EXPECT: print("         BUF MISMATCH exp %s"%" ".join("%02x"%x for x in EXPECT))
if tiles!=TILE_EXPECT: print("         TILE MISMATCH")
print("\n>>> TWO-SOURCE VECTORED DISPATCH %s <<<"%("PASSED" if ok else "FAILED"))
