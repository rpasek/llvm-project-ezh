...
int main(void) {
  ...
  /* --- private SDR write to the freshly assigned dynamic address --- */
  R(MCTRL) = REQ_EMITSTART | TYPE_I3C | ADDR(DYN_ADDR);
  ev_hold(ST_MCTRLDONE);
  if (R(MSTATUS) & ST_NACKED)
    m_nacked |= 4u;
  R(MSTATUS) = ST_MCTRLDONE | ST_NACKED;
  m_stage = 2;

  /* 17 bytes, each gated by the TXNOTFULL interrupt (i3c_stream idiom) */
  R(MINTCLR) = 0xFFFFu;
  R(MINTSET) = ST_TXNOTFULL;
  __builtin_ezh_write_cfm(cfm);
  __builtin_ezh_hold();
  R(MWDATAB) = 0x00u;
  __builtin_ezh_write_cfm(cfm);
  ...
  __builtin_ezh_hold();
  R(MWDATABE) = 0x1Fu; /* END -> STOP */
  m_holds++;
  m_stage = 3;

  ev_hold(ST_COMPLETE);
  R(MSTATUS) = ST_COMPLETE | ST_MCTRLDONE;
  stop_if_active();
  m_complete = 1u;
  m_stage = 4;
