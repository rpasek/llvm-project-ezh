...
|---|---|---|---|
| 1 | **Build & hygiene** | PassManagerBase forward-decl fix; `-Werror`-clean backend and lldb plugin; dead STATISTIC removal; predicated pushd/popd alias printing -- CARVED (`series/1`, 5 commits): the printer fix ships with a new MC round-trip test over the raw codegen-shaped `ldr_post`/`str_pre` forms (mutation-verified; the suffixed-alias asm forms never reached the buggy hand-rolled path), because the original `predicated-pop.ll` only if-converts after series 5's mul decomposition and moves there | `7e766844`, `3c233b24`, `e4ed4d91`, `932b3386`, `13d76eb0` |
...
| 4 | **Intrinsic/builtin surface** | The `__builtin_ezh_*` layer EXCEPT `tight_loop`: GPIO, event fabric, CFM/CFS, GPO, vectored holds (incl. the hardware-writes-RA `Defs=[RA]` fact); the `getArchTypePrefix` fix that unblocks all of it; Sema immediate ranges. (`__builtin_ezh_tight_loop` travels with series 11: exposing the intrinsic without the constant-island region protection recreates the hand-written-loop wrong-code window, and its contract would claim a protection that does not exist at this tip.) | `8753a819`, `7fd4b74f`, `1dd6cd5b`, `1b04f9e3`, `4ecd0901`, `2307e872`, `c22824c4`, `149a0350` |
| 5 | **Codegen quality I** | Frame elision (no phantom FP), EZHCompareFusion (+ -g/optnone hardening), constant-multiply decomposition + shift-add chains, sign-bias compare fold, 0/1 boolean contents; carries `predicated-pop.ll` from `13d76eb0` (its shape needs mul decomposition to if-convert) | `d075e8b0`, `f4f39ab9`, `934017f5`, `8233d8d8`, `2623fa72`, `ed6c60f1`, test from `13d76eb0` |
...
| 11 | **tight_loop hardware loops** | The complete, self-contained tight_loop unit: the `__builtin_ezh_tight_loop` intrinsic/builtin exposure with the silicon-TRUE contract (the instruction itself is already in the base -- only exposure lands here, and it never ships without its protection); EZHTightLoopFormation (n≥1 evidence rules, CFS discipline, budgets) with .ll + MIR evidence tests (incl. the de-vacuated `guard_taken_path` from `4bd729f4`); constant-island superset-invariant region protection (+ `-run-pass` registration, unresolved-Rend MIR test); the rotation-is-worthless verdict comment | `009c6d35`, contract part of `ad00de20`, `8a0c9102`, `cbe16be3`, rest of `42476f38`, `cd371f75`, evidence-test part of `4bd729f4`, `e7de9d49`, island/timing parts of `ad00de20`, `7be2cbf8`, `59e46c5c` |
...
2. tight_loop is a FREE-RUNNING counted loop with a run-once slot after the
   ...
   event-paced theory is disproven. (Series 11; AN14650 + cycle timing.)
